...
function initialisation() {
  ...
  if (todoItems.length === 0) {
    console.log(document.getElementById("no-item-text"));
    console.log(todoItems);
    document.getElementById("no-item-text").style.display = "block";
    
  } else {
    console.log("inside");
    document.getElementById("no-item-text").style.display = "none";
  }
}
...
function renderTodo(){
  ...
  initialisation();
  const list = document.querySelector(".card-item-container");
  ...
  for (let i = 0; i < todoItems.length; i++) {
    ...
    node.innerHTML = `<p class="card-heading" onclick="redirect(this)">${todoItems[i].heading}</p>
      ...
      <div class='footer'>
          <button class='btn-completed' onclick="removeToDo(this)"><i class="fa fa-trash"></i></button> 
